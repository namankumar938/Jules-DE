...
## Directory Structure
*   `config.yaml`: Main configuration file for the pipeline settings.
*   `pyspark_pipeline/`: Directory containing the core PySpark pipeline modules.
    ...
    *   `config_utils.py`: Utility for loading configurations from the `config.yaml` file.
...
## Configuration
The pipeline's behavior is controlled by settings in the `config.yaml` file, located in the project root. This file allows you to customize parameters without modifying the source code.

Key configurable parameters include:
*   **`application.name`**: The name of the Spark application.
*   **`logging.level`**: The default logging level for the application (e.g., INFO, DEBUG, WARNING).
*   **`input_data.file_path`**: The path to the input CSV data file (relative to the project root).
*   **`processing_rules.transformations.allowed_categories`**: A list of product categories that are considered valid during data cleaning.
*   **`processing_rules.transformations.allowed_payment_methods`**: A list of payment methods that are considered valid.
*   **`processing_rules.kpi_generation.top_selling_products.top_n_default`**: The default number of products to display in the "Top Selling Products" KPI.

The `pyspark_pipeline/main.py` script loads this configuration file at startup using utilities from `config_utils.py`.
...
*   **Dependencies:**
    ...
    *   `PyYAML`: For parsing the `config.yaml` file. Install using `pip install PyYAML`.
*   **Environment:**
...
2.  Execute the main pipeline script using:
    ...
    *   The script uses settings from `config.yaml` (e.g., for input file path, processing rules).
...
The default logging level for the pipeline is set in `config.yaml` under the `logging.level` key (e.g., "INFO", "DEBUG"). Modify this value in `config.yaml` to change the verbosity of log output. For example, setting it to "DEBUG" will provide more detailed logs, including samples of raw and cleaned DataFrames if the `main.py` script's DEBUG conditional displays are enabled.

## Modules Overview (`pyspark_pipeline/`)

*   **`main.py`**: Initializes the `SparkSession`, loads configurations from `config.yaml`, and orchestrates the pipeline's execution flow. It calls functions from ingestion, transformations, and kpi_generation modules (passing necessary configurations) in sequence and prints the final KPI results.
...
*   **`transformations.py`**: Contains the `clean_and_transform_data` function, which applies various data cleaning rules to the DataFrame. This includes handling null or invalid values, normalizing string data, and filtering records based on configurable business rules (e.g., allowed categories and payment methods passed as parameters).
*   **`kpi_generation.py`**: Houses functions to compute the defined Key Performance Indicators. Each function takes the cleaned Spark DataFrame and relevant configuration parameters (like allowed categories/payment methods for structuring results, or `top_n` for limiting results) as input and performs necessary aggregations and calculations.
...
*   **`config_utils.py`**: Contains utility functions to load and parse the `config.yaml` file, making configurations accessible to the pipeline modules.

## How It Works (High-Level PySpark Flow)
1.  **Configuration Loading**: At startup, pipeline configurations (e.g., file paths, processing rules, logging level) are loaded from `config.yaml` by `main.py` via `config_utils.py`.
2.  **SparkSession Initialization**: A `SparkSession` is created, serving as the entry point for Spark functionality (`main.py`).
3.  **Data Ingestion**: Purchase data is loaded from the configured CSV file into a Spark DataFrame. An explicit schema is applied during this stage to define data types for each column (`ingestion.py`).
4.  **Data Transformation**: The raw DataFrame is then processed by the `clean_and_transform_data` function in `transformations.py`, using configurable rules (e.g., allowed categories) passed from `main.py`. This step involves filtering, validating, and normalizing data.
5.  **KPI Calculation**: The cleaned and transformed DataFrame is passed to various functions in `kpi_generation.py`, along with relevant configurations (e.g., `top_n` value, lists of allowed categories/payment methods for complete reporting). These functions use Spark's distributed computation capabilities to calculate each KPI.
6.  **Results Display**: The resulting KPI DataFrames (and the scalar Average Order Value) are then printed to the console for review (`main.py`).
7.  **SparkSession Stop**: Finally, the `SparkSession` is stopped to release resources.
